...
export function useCompass(
  ...
  const dx = computed(() =>
    active.value
      ? game.value.chest.x - (originX.value + viewportWidth.value / 2)
      : 0,
  )
  const dy = computed(() =>
    active.value
      ? game.value.chest.y - (originY.value + viewportHeight.value / 2)
      : 0,
  )
  ...
  const angleDeg = computed(
    () => (Math.atan2(dx.value, -dy.value) * 180) / Math.PI,
  )
